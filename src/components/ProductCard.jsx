...
const ProductCard = ({ product, category, subcategory }) => {
      ...
      return (
        <button className="btn btn-outline-primary d-flex align-items-center justify-content-center gap-2 w-100">
          <div
            ...
            style={{ padding: ".2em", width: "2rem", height: "2rem" }}
          >
            <FontAwesomeIcon icon={faMinus} />
          </div>
          <span style={{ margin: "0 10px" }} className="text-black fw-bold">
  ...
  const [liked, setLiked] = useState(isLikedWishItems(product.productId));
  ...
  useEffect(()=>{
    ...
    // getCurrentUser()
  if (cuser) {
    const wishlistKey = `wishlist_${cuser.id}`;
    ...
    setLiked(existingWishlist.some((wishlistItem) => wishlistItem.productId === product.productId))
    // return existingWishlist.some((wishlistItem) => wishlistItem.productId === product.productId);
  }else{
    setLiked(false)
  }
